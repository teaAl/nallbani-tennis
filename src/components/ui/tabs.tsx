...
export function TabsList({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`flex rounded-lg bg-gray-800 p-1 ${className}`}>{children}</div>
}
  ...
  return (
    <button
      className={`flex-1 rounded-md px-3 py-1.5 text-sm cursor-pointer transition-all font-nunito ${
        isActive ? "bg-pear text-gray-900 shadow-sm " : "text-foreground/50 font-medium hover:text-foreground/100"
      } ${className}`}
