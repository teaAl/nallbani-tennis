  ...
  return (
    <footer className=" text-sm text-pear text-center bg-[linear-gradient(to_right,rgba(16,24,40,0.9),rgba(16,24,40,0.9)),url('/images/parallax-4.jpg')] bg-fixed bg-bottom bg-no-repeat relative">
      <div className="hidden md:flex flex-row gap-2 items-center justify-start bg-gradient-to-r from-foreground/30 to-pear/40 w-max py-1 px-2 rounded-r-3xl absolute top-2 left-0">
        ...
        <span className=" text-foreground text-md font-poppins">
          {t("chatwhap")}
        </span>
      </div>
      <div className="flex md:w-3/4 w-full flex-row flex-wrap justify-between gap-6 md:items-center items-start md:mx-auto md:p-10 p-5">
        <div className="flex flex-col gap-2 items-center md:w-max w-full">
          <div className="hidden md:block">
            <Image
              ...
              className={`${
                isLogoVisible
                  ? "animate-fade-right animate-once animate-ease-linear"
                  : "opacity-0"
              }`}
              ref={logoRef}
            />
          ...
          <div
            className={`flex flex-col gap-0.5 items-center
                        ${
                          isLogoTitleVisible
                            ? "animate-fade animate-once animate-ease-linear delay-1000"
                            : "opacity-0"
                        }`}
            ref={logoTitle}
          >
            <p className="font-poppins font-light text-lg tracking-widest">
              Nallbani <span className="text-white">Tennis</span>
            </p>
        ...
        <div
          className={`md:hidden block w-full
                    ${
                      isMobileBtnVisible
                        ? "animate-fade animate-once animate-ease-linear delay-500"
                        : "opacity-0"
                    }`}
          ref={mobileBtn}
        >
        ...
        <div className="flex flex-col gap-4 justify-around w-max">
          <h5
            className={`uppercase text-white md:border-b-2 border-b border-b-white/30 py-1 text-left font-bold md:text-lg text-base
                        ${
          ...
          >
            <div className="flex flex-row gap-2 items-center">
              <EnvelopeIcon />
              <span className="text-white">nallbanitennis@gmail.com</span>
            </div>
            <div className="flex flex-row gap-2 items-center">
              <LocationIcon />
            ...
            <div className="flex flex-row gap-2 items-center">
              <PhoneIcon />
              <span className="text-white">+355 69 123 4567</span>
            </div>
          </div>
        </div>
        <div
          className={`md:hidden flex flex-col gap-7 justify-between items-end w-1/3 mb-2 mt-auto
                    ${
                      isSocialsMobileVisible
                        ? "animate-fade-up animate-once animate-ease-linear"
                        : "opacity-0"
                    }`}
        ...
        <div className="hidden md:flex flex-col gap-6 justify-between md:w-max w-max">
          ...
          <div
            className={`group ${
              isButtonVisible
                ? "animate-fade-left animate-once animate-ease-linear delay-700"
                : "opacity-0"
            }`}
            ref={buttonRef}
          >
