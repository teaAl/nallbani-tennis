...
const LanguageSwitcher = () => {
  ...
  return (
    <>
      <div
        className="hidden md:flex flex-row gap-2 items-center cursor-pointer hover:scale-105 transition-all duration-300"
        onClick={() => setShowDropdown(!showDropdown)}
      >
        {language === "en" ? <EnIcon size="25" /> : <AlIcon size="25" />}
        <span className="text-foreground uppercase">{language}</span>
      </div>
      <div
        className={`${
          showDropdown ? "absolute top-12 right-1.5 z-50" : "hidden"
        } bg-gray-900 rounded-md gap-2 `}
      >
        <ul className="flex flex-col">
          {language === "en" ? (
            <li
              ...
              onClick={() => {
                changeLocale("al"), setShowDropdown(false);
              }}
            >
              <AlIcon size="25" />
            ...
            <li
              ...
              onClick={() => {
                changeLocale("en"), setShowDropdown(false);
              }}
            >
              <EnIcon size="25" />
      ...
      <div
        className="md:hidden flex flex-row gap-2 items-center cursor-pointer"
        onClick={() => handleLanguageMobile()}
      >
        {language === "en" ? <EnIcon size="30" /> : <AlIcon size="30" />}
